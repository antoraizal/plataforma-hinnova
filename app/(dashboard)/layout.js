...
export default function DashboardLayout({ children }) {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-[#fffaf3] text-[#526171]"><div className="text-center"><div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-4 border-[#dcebf5] border-t-[#1f6aa5]" /><p>Preparando tu espacio HINOVA…</p><p className="mt-2 text-xs text-[#718094]">Restaurando tu sesión de forma segura</p></div></div>;
  if (!session) return <div className="flex min-h-screen items-center justify-center bg-[#fffaf3] p-5"><div className="card w-full max-w-sm text-center"><img src="/logo.png" alt="HINOVA" className="mx-auto h-14 w-36 object-contain" /><h1 className="mt-4 text-lg font-bold text-[#102b4e]">Tu sesión necesita actualizarse</h1><p className="mt-2 text-sm text-[#718094]">Vuelve a ingresar para continuar usando HINOVA.</p><Link href="/login" className="btn-primary mt-5 inline-flex">Ir al inicio de sesión</Link></div></div>;
